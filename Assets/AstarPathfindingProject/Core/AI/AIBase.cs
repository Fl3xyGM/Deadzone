...
namespace Pathfinding {
	...
	public abstract class AIBase : VersionedMonoBehaviour {
		...
		protected virtual void Update () {
			...
			GetComponent<AIPath>().maxSpeed = GetComponent<AIDestinationSetter>().MaxSpeed;;
		...
		void FinalizeRotation (Quaternion nextRotation) {
			...
			Quaternion ShittyFuckingGarbageRotation = Quaternion.Euler(0, 0, nextRotation.eulerAngles.z+90f);

			GetComponent<AIDestinationSetter>().transform.Find("gfx").transform.rotation = ShittyFuckingGarbageRotation;
